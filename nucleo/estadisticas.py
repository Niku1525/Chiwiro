...
Se guarda en estadisticas/<guild_id>.json:
...
def _ruta(guild_id: int) -> str:
    return os.path.join(almacen.carpeta("estadisticas"), f"{guild_id}.json")
...
def registrar(guild_id: int, titulo: str, url: str, pedida_por: str) -> None:
    ...
    try:
        datos = almacen.leer(_ruta(guild_id), {})
        ...
        almacen.guardar(_ruta(guild_id), datos)
    except Exception:
        pass


def top_canciones(guild_id: int, limite: int = 10) -> list[dict]:
    canciones = almacen.leer(_ruta(guild_id), {}).get("canciones", {})
    ordenadas = sorted(
...
def top_usuarios(guild_id: int, limite: int = 10) -> list[tuple[str, int]]:
    usuarios = almacen.leer(_ruta(guild_id), {}).get("usuarios", {})
